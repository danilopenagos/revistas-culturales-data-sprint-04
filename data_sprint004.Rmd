---
title: "Red de revistas colombianas de la primera mitad del siglo XX"
output: 
  html_notebook: 
    toc: yes
    code_folding: hide
...
# Modelado de datos para el análisis de redes en revistas colombianas
...
```{r Anónimos y autorías colectivas}
...
En este punto, el objetivo es comparar el modelo de contribudores limpio (con datos biográficos) y el modelo de las 12 revistas con las que estamos trabajando.
...
```{r Creación data set único de colaborares encontrados}
...
```{r Colaboradores más prólificos}
# Esxtraemos los primeros 25 colaboradores
...
```{r Visualización colaboradores más prólificos}
# ppi <- 300
...
colabs_mas_prolificos |>
  ...
  theme(panel.background = element_rect(fill = "white"),
    ...
    geom_text(
      data = subset(colabs_red, Frecuencia >= 35),
...
```{r Colaboradores con mejores conexiones}
...
```{r Visualización autores mejores conectados}
...
```{r Social Network analysis: aristas}
...
```{r Social Network analysis: creación de nodos}
...
```{r Social Network analysis: nodos alternativos}
...
```{r Social Network analysis: nodos alternativos para análisis}
...
```{r Cálculo de la centralidad por medio de la betweenness}

# # copy the existing node list into a new object
# nodos <- nodos_completo_alt
# 
...
```{r Carga de datos revistas modernistas hispanoamericanas}
...
# 3918 edges remain; Guardamos el resultado
...
```{r Creación de nodos revistas modernistas}
...
```{r Creación nodos alternativos modernismo}
...
```{r Cálculo de la modularidad red modernista}
...
# Mostramos el resultado
modularity(red_revista, membership(modularidad_modernismo), directed = T)
# 0.6224363
...
# Añadimos una columna con este dato a nuestra tabla de nodos como grupos ('group')
nodos_vis_modernismo$group <- V(red_revista)$community
...
```{r Visualización red modernista}
